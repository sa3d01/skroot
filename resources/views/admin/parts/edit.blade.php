    ...
    <div class="container-fluid">
        <div class="animated fadeIn">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header"><h4>{{ __('Edit') }}: {{ $part->name }}</h4></div>
                        <div class="card-body">
                            <form method="POST" action="{{ route('admin.parts.update',$part->id) }}">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control @error('part_category_id') is-invalid @enderror"
                                            ...
                                            @foreach($categories as $category)
                                                <option {{$part->part_category_id == $category->id?" selected ":""}}
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control @error('car_brand_id') is-invalid @enderror"
                                            ...
                                            @foreach($brands as $brand)
                                                <option {{$part->car_brand_id == $brand->id?" selected ":""}}
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control @error('car_brand_model_id') is-invalid @enderror"
                                            ...
                                            @foreach($models as $model)
                                                <option {{$part->car_brand_model_id == $model->id?" selected ":""}}
                                ...
                                <a href="{{ route('admin.parts.index') }}" class="btn btn-primary">{{ __('Return') }}</a>
